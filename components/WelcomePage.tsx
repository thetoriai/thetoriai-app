...
export const WelcomePage: React.FC<WelcomePageProps> = ({
  ...
  return (
    <div className="w-full h-full flex flex-col items-center bg-gray-940 overflow-y-auto overflow-x-hidden relative perspective-1000 scrollbar-none">
      ...
      <div
        className={`fixed inset-0 z-0 overflow-hidden pointer-events-none transition-all duration-1000 ${
          isMinimalView ? "scale-105 saturate-[1.1]" : ""
        }`}
      >
        <video
          ref={bgVideoRef}
      ...
      <div
        className={`relative z-20 flex flex-col items-center text-center max-w-5xl px-6 pt-16 transition-all duration-700 ${
          isMinimalView
          ...
          {[
            {
              icon: UserPlusIcon,
              title: "Consistency",
              desc: "Identity-locked actors across scenes.",
              color: "text-indigo-400"
            },
            {
              icon: SparklesIcon,
              title: "High-Fidelity",
              desc: "Generative 3D visual blueprints.",
              color: "text-amber-400"
            },
            {
              icon: VideoIcon,
              title: "Workflow",
              desc: "Timeline sequencing & master export.",
              color: "text-emerald-400"
            }
          ].map((feature, i) => (
            <div
              key={i}
              className="flex flex-col items-center p-5 bg-white/[0.01] border border-white/5 rounded-2xl backdrop-blur-xl group hover:bg-white/[0.03] transition-all cursor-default"
            >
              <feature.icon
          ...
          <div className="max-w-7xl mx-auto flex flex-col gap-6 animate-in slide-in-from-bottom-10 duration-1000 delay-700">
            ...
            <div className="grid grid-cols-4 gap-6">
              {PLACEHOLDER_SLOTS.map((slot) => (
                <div
                  key={slot}
                  className="aspect-video bg-black/20 rounded-xl border border-dashed border-white/5 flex flex-col items-center justify-center group hover:border-white/10 transition-all opacity-40 hover:opacity-100"
                >
      ...
      {isMobile && !isMinimalView && (
        <div className="relative z-30 px-4 mt-4 pb-12">
          <div className="flex gap-4 overflow-x-auto pb-6 scrollbar-none snap-x">
            {[
              ...REEL_VIDEOS,
              ...PLACEHOLDER_SLOTS.map(() => REEL_VIDEOS[0])
            ].map((src, i) => (
              <div
                key={i}
                className="min-w-[240px] aspect-video bg-black/40 rounded-xl border border-white/10 overflow-hidden snap-center"
              >
                {i < 4 ? (
                  <video
                    src={src}
