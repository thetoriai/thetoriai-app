        ...
        <div className={!isOverlay ? "min-h-screen bg-gray-950 flex items-center justify-center p-4 relative overflow-hidden" : ""}>
            ...
            <div className={`${containerClasses} relative z-10`}>
                {onBack && (
                    <button 
                        onClick={onBack}
                        className="absolute top-8 left-8 text-gray-400 hover:text-white transition-colors flex items-center gap-1 text-[10px] font-black  tracking-widest"
                    >
                ...
                <div className="flex flex-col items-center mb-8 text-center mt-6">
                    ...
                    <p className="text-indigo-400/60 text-[8px] font-black  tracking-[0.5em] mt-3">Identity Verification System</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                    ...
                    >
                        ...
                        <span className="text-[9px] font-black  tracking-widest text-gray-300 group-hover:text-white">Gmail</span>
                    </button>
                    <button 
                    ...
                    >
                        ...
                        <span className="text-[9px] font-black  tracking-widest text-gray-300 group-hover:text-white">Facebook</span>
                    </button>
                </div>

                <div className="flex items-center gap-4 mb-6 opacity-20">
                    <div className="h-px flex-1 bg-white"></div>
                    <span className="text-[8px] font-black  tracking-widest text-white">OR</span>
                    <div className="h-px flex-1 bg-white"></div>
                </div>

                <div className="flex bg-white/5 rounded-2xl p-1 mb-8 border border-white/5">
                    <button onClick={() => { setMode('signin'); setError(null); setMessage(null); setShowSignUpHint(false); }} className={`flex-1 py-2.5 text-[9px] font-black  tracking-widest rounded-xl transition-all ${mode === 'signin' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}>Sign In</button>
                    <button onClick={() => { setMode('signup'); setError(null); setMessage(null); setShowSignUpHint(false); }} className={`flex-1 py-2.5 text-[9px] font-black  tracking-widest rounded-xl transition-all ${mode === 'signup' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}>Register</button>
                </div>

                <form onSubmit={handleAuth} className="space-y-5">
                    <div className="space-y-1.5">
                        <label className="text-[8px] font-black text-gray-500  tracking-widest block ml-2">Production Email</label>
                        <input 
                            type="email"
                            value={email}
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[8px] font-black text-gray-500  tracking-widest block ml-2">Access Key</label>
                        <input 
                            type="password"
                            value={password}
                        ...
                        <div className="p-3 bg-red-900/10 border border-red-500/20 rounded-2xl flex items-start gap-2 animate-in slide-in-from-top-1">
                            ...
                            <div className="flex-1">
                                ...
                                {showSignUpHint && mode === 'signin' && (
                                    <p className="text-[9px] text-indigo-400 mt-2 font-black  cursor-pointer hover:underline" onClick={() => { setMode('signup'); setError(null); setShowSignUpHint(false); }}>
                                        Register new ID?
                                    </p>
                                )}
                            </div>
                    ...
                    <button 
                        type="submit" 
                        disabled={loading}
                        className="w-full py-5 bg-indigo-600 hover:bg-indigo-500 text-white font-black  tracking-[0.3em] rounded-3xl shadow-2xl disabled:bg-gray-800 disabled:text-gray-700 flex items-center justify-center gap-3 transition-all active:scale-[0.98] text-[10px] mt-6 group border border-white/10"
                    >
                    ...
                    {message && (
                        <div className="p-4 bg-green-900/20 border border-green-500/30 rounded-2xl flex items-center gap-3 animate-in slide-in-from-top-2 mt-4">
                            ...
                            <p className="text-[10px] text-green-400 font-bold leading-tight  tracking-wider">{message}</p>
                        </div>
                    )}
                </form>

                <div className="mt-8 pt-6 border-t border-white/5 flex flex-col items-center gap-2">
                    {/* Fixed: Removed  class to allow lowercase email display */}
                    <p className="text-[8px] font-black text-gray-500 tracking-widest">Technical Assistance: {SUPPORT_EMAIL}</p>
                    <p className="text-[7px] text-gray-700 text-center leading-tight  font-black tracking-[0.4em]">
