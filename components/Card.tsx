...
import { DownloadIcon, RefreshIcon, BookmarkIcon, CameraIcon, SparklesIcon, VideoIcon, TrashIcon, PlusIcon, ClapperboardIcon, XIcon, ExclamationTriangleIcon, StopIcon, UndoIcon, CircularProgressIcon, ChevronLeftIcon, ChevronRightIcon, PlayIcon, CreditCardIcon, ChevronDownIcon, PhotoIcon, SpeakerWaveIcon } from './Icons';
...
export const SceneProgressOverlay: React.FC<SceneProgressOverlayProps> = ({ onStop, label = "Producing..." }) => {
...
export interface SceneCardProps {
    ...
    onPreviewImage: (src: string) => void; onSave: () => void; onAngle: () => void; onEdit: () => void; onRegenerate: (genId: number, sceneId: string) => void; onDelete: () => void; onUndo: () => void; onVariantChange: (dir: 'next' | 'prev') => void; onStopScene: () => void; onToggleVideoCreator: () => void;
    onUpdateDraft: (updates: any) => void; onGenerateVideo: (script: string, movement: string, withAudio?: boolean) => void; onAddToTimeline: (url: string, duration?: number, obj?: any) => void; onImportScript: () => void; hasScriptToImport: boolean;
    ...
    isMusicVideo: boolean; isHistory: boolean; isEnhanced?: boolean; videoError?: string; isConfirmingVideo: boolean; creditBalance: number;
...
export const SceneCard: React.FC<SceneCardProps> = (props) => {
    ...
    const baseVideoCost = props.videoModel === 'veo-3.1-fast-generate-preview' ? 5 : 8;
    ...
    // DO add comment: Added Safety Block UI Logic.
                    ...
                    <>
                        {showVideoPlayer && currentVideo ? (
                            <div className="w-full h-full relative group/video">
                                <video ref={videoRef} src={currentVideo.videoUrl} controls autoPlay loop className="w-full h-full object-contain" />
                                <button onClick={() => props.onAddToTimeline(currentVideo.videoUrl, videoRef.current?.duration, currentVideo.videoObject)} className="absolute top-2 right-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold rounded-lg shadow-xl opacity-0 group-hover/video:opacity-100 transition-opacity z-20 flex items-center gap-1">Add to timeline</button>
                            </div>
                        ) : scene.src ? (
                            ...
                            <div className="absolute inset-0 bg-amber-950/20 backdrop-blur-sm p-6 flex flex-col items-center justify-center text-center animate-in fade-in duration-500">
                                ...
                                <h4 className="text-[10px] font-black text-amber-500 uppercase tracking-[0.2em] mb-2 leading-none">
                                    {isMinorBlock ? 'Production Advisory: Minor Detected' : 'Content Rejected: Explicit'}
                                </h4>
                        ...
                        ) : <div className="text-center text-red-400 p-4 font-bold text-[10px] tracking-widest leading-relaxed"><ExclamationTriangleIcon className="w-8 h-8 mx-auto mb-2 opacity-50"/> Signal Lost</div>}
                    </>
                )}
            </div>
                    ...
                    {isActive ? (
                        <div className="p-3 bg-gray-900/50 space-y-3 animate-in slide-in-from-top-4 relative z-50 rounded-b-none border-b-0">
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                                <button onClick={() => setDisplayMode('image')} className={`shrink-0 w-12 h-10 rounded-lg border overflow-hidden transition-all ${displayMode === 'image' ? 'border-indigo-500 ring-2 ring-indigo-500/50 opacity-100 shadow-lg' : 'border-gray-700 opacity-60 hover:opacity-100 hover:bg-gray-700'}`}><img src={formatImageSrc(scene.src)} className="w-full h-full object-cover" /></button>
                                {videoState?.clips?.map((clip: any, idx: number) => (
                                    <button key={idx} onClick={() => setDisplayMode(idx)} className={`shrink-0 w-12 h-10 rounded-lg border overflow-hidden transition-all bg-black ${displayMode === idx ? 'border-indigo-500 ring-2 ring-indigo-500/50 opacity-100 shadow-lg' : 'border-gray-700 opacity-60 hover:opacity-100 hover:bg-gray-700'}`}><div className="relative w-full h-full"><video src={clip.videoUrl} className="w-full h-full object-cover" /><div className="absolute inset-0 flex items-center justify-center bg-black/30"><PlayIcon className="w-3 h-3 text-white"/></div></div></button>
                                ))}
                            </div>
                            ...
                            <div className="flex items-end gap-1.5">
                                <div className="flex-1">
                                    <label className="text-[8px] font-bold text-gray-600 mb-0.5 block tracking-widest px-1 uppercase">Camera Movement</label>
                            ...
                            <div className="flex gap-1 mt-4">
                                <button 
                                    onClick={() => props.onGenerateVideo(props.draftScript, props.draftMovement, withAudio)} 
                                    disabled={props.isDisabled || isVideoLoading} 
                                    className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-xs font-black tracking-[0.2em] uppercase rounded-l-none rounded-r-none transition-all text-white shadow-xl ${ props.isConfirmingVideo ? 'bg-green-600 hover:bg-green-700 scale-[1.02]' : 'bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-800 disabled:text-gray-600' }`}
                                >
                                    {isVideoLoading ? "Producing..." : props.isConfirmingVideo ? `Confirm (${totalCompoundCost}C)` : <><ClapperboardIcon className="w-4 h-4" /> Produce</>}
                                </button>
