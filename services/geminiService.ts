...
async function withRetry<T>(apiCall: () => Promise<T>, onRetryMessage?: (msg: string) => void, signal?: AbortSignal): Promise<T> {
    ...
    while (attempt < maxRetries) {
        ...
        try {
            ...
            const isRetryable = errorMessage.includes('503') || errorMessage.includes('429') || errorMessage.includes('500');
            if (isRetryable && attempt < maxRetries) {
                const delaySeconds = Math.pow(2, attempt) * 10;
                if (onRetryMessage) onRetryMessage(`Model Busy. Retrying in ${delaySeconds}s...`);
...
function getStyleInstructions(style: string, characterStyle: string = 'General'): string {
    const isAfrican = characterStyle === 'Afro-toon';
    const africanMandate = isAfrican ? " MANDATORY AFRICAN SETTING: This scene takes place ENTIRELY inside Africa. Use authentic African architecture, landscapes, and local cultural markers." : "";
...
export async function generateCharacterDescription(imageBase64: string, mimeType: string, signal?: AbortSignal): Promise<{ description: string; detectedStyle: string }> {
    ...
    return { description: parsed.description || '', detectedStyle: parsed.detectedStyle || 'Realistic' };
}

export async function editImage(params: EditImageParams): Promise<{ src: string | null; error: string | null }> {
    const ai = getAiClient();
    const { imageBase64, mimeType, editPrompt, aspectRatio, visualStyle, characterStyle, characters, hasVisualMasks, signal, imageModel, overlayImage, referenceImage } = params;
    
    const parts: Part[] = [
        { inlineData: { data: stripBase64Prefix(imageBase64), mimeType: detectMimeType(imageBase64) } }
    ];

    if (overlayImage) {
        parts.push({ inlineData: { data: stripBase64Prefix(overlayImage.base64), mimeType: overlayImage.mimeType } });
    }
    
    if (referenceImage) {
        parts.push({ inlineData: { data: stripBase64Prefix(referenceImage.base64), mimeType: referenceImage.mimeType } });
    }
    ...
    try {
        ...
        if (msg.includes('safety') || msg.includes('blocked')) return { src: null, error: "BLOCK_SAFETY_GENERAL" };
        return { src: null, error: e.message };
    }
}

export async function generateSingleImage(prompt: string, aspectRatio: string, characterStyle: string, visualStyle: string, genre: string, characters: Character[], model: string = 'gemini-3-pro-image-preview', referenceImage?: string | null, historyImage?: string | null, signal?: AbortSignal): Promise<{ src: string | null; error: string | null }> {
    const ai = getAiClient();
    const parts: Part[] = [];
    
    if (referenceImage) {
        parts.push({ inlineData: { data: stripBase64Prefix(referenceImage), mimeType: 'image/png' } });
    }
    if (historyImage) {
        parts.push({ inlineData: { data: stripBase64Prefix(historyImage), mimeType: 'image/png' } });
    }
...
export async function generateCharacterVisual(character: Character, uiSelectedStyle: string, characterStyle: string, selectedCountry: string, signal?: AbortSignal): Promise<{ src: string | null; error: string | null }> {
    const customNotes = character.customInstruction ? ` INSTRUCTION: ${character.customInstruction}.` : "";
    
    const isAfro = characterStyle === 'Afro-toon';
    const ethnicityMandate = isAfro ? "ETHNICITY MANDATE: The subject MUST be a person of African descent (Black person) with authentic features." : `ETHNICITY CONTEXT: The subject should match the predominant ethnicity of ${selectedCountry}.`;
    
    const hasClothingInstruction = character.customInstruction?.toLowerCase().includes('shirt') || character.customInstruction?.toLowerCase().includes('clothes') || character.customInstruction?.toLowerCase().includes('wear') || character.customInstruction?.toLowerCase().includes('outfit');
    
    let casualWearPrompt = "";
    if (!hasClothingInstruction) {
        if (selectedCountry === 'Nigeria') {
            casualWearPrompt = "WARDROBE: Contemporary Nigerian casual wear (e.g., a modern Ankara-print shirt or a stylish modern Kaftan shirt).";
        } else if (selectedCountry === 'USA') {
            casualWearPrompt = "WARDROBE: Modern American casual wear (e.g., high-quality jeans and a stylish t-shirt).";
        } else if (selectedCountry === 'UK') {
            casualWearPrompt = "WARDROBE: Modern British casual wear (e.g., a smart jumper or stylish hoodie).";
        } else {
...
export async function generateVideoFromScene(scene: any, aspectRatio: string, prompt: string, image: string | null, style: string, characterStyle: string, model: string, resolution: '720p' | '1080p', cameraMovement: string, onRetry?: (msg: string) => void, characters?: Character[]): Promise<{ videoUrl: string | null; videoObject: any }> {
    const ai = getAiClient();
    const castNotes = characters?.map(c => `${c.name}: ${c.description}`).join('; ') || "";
    const fullPrompt = `Style: ${style}. Character Context: ${castNotes}. Action: ${prompt}. Camera: ${cameraMovement}.`;
    ...
    try {
        ...
        if (downloadLink) {
            ...
            return { videoUrl: fetchUrl, videoObject: operation.response?.generatedVideos?.[0]?.video };
        }
...
export async function generateStructuredStory(idea: string, title: string, characters: Character[], includeDialogue: boolean, characterStyle: string, genre: string, movieStyle: string, numScenes: string, history: string, isMusicVideo: boolean, songLyrics: string, country: string): Promise<any> {
    const ai = getAiClient();
    const castNotes = characters.map(c => `${c.name}: ${c.description}`).join('; ');
...
export async function generateScenesFromNarrative(narrative: string, characters: Character[], includeDialogue: boolean, characterStyle: string, movieStyle: string, country: string): Promise<any[]> {
    const ai = getAiClient();
    const castNotes = characters.map(c => `${c.name}: ${c.description}`).join('; ');
...
export async function regenerateSceneVisual(script: string, characters: Character[]): Promise<string> {
    const ai = getAiClient();
    const castNotes = characters.map(c => `${c.name}: ${c.description}`).join('; ');
...
export async function generatePromptFromAudio(base64: string, mimeType: string): Promise<string> {
    const ai = getAiClient();
    const response: GenerateContentResponse = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: [
...
export async function generateSpeech(text: string, country: string, voice: string, expression: string): Promise<string> {
    const ai = getAiClient();
